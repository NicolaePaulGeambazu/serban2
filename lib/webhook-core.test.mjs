...
// Capture console output so success-log lines are asserted, not leaked into test output.
function capture() {
  const logs = [], errs = [];
...
test('handleWebhook uploads a click conversion on order_add and logs a success line', async () => {
  const client = fakeClient();
  const cap = capture();
  let r;
  try {
...
test('handleWebhook retracts on a canceled update and logs a success line', async () => {
  const client = fakeClient();
  const cap = capture();
  let r;
  try {
...
test('handleWebhook restates value on an approving update and logs a success line', async () => {
  const client = fakeClient();
  const cap = capture();
  let r;
  try {
  ...
  assert.match(cap.logs[0], /restate ok order=OR1 20\.5 RON/);
});
