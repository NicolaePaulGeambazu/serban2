...
// Log every money-moving Google Ads call: one line per event. A failure (non-2xx
// or a partial-failure body) is an error line — the ONLY signal, since we still
// return 200 to ProfitShare; a success is an info line so the logs read as an audit trail.
function reportResult(label, detail, r) {
  if (!r || r.status < 200 || r.status >= 300 || r.json?.partialFailureError) {
    console.error('profitshare-webhook: Google Ads', label, 'failed', r?.status, detail, JSON.stringify(r?.json?.partialFailureError ?? r?.json));
  } else {
...
export async function handleWebhook({ query, headers, env, client, now = new Date() }) {
  ...
  if (status === 'canceled' || status === 'cancelled') {
    ...
    reportResult('retract', `order=${query.order_reference}`, r);
    return { status: 200, action: 'retract' };
  }
  if (type === 'order_add') {
    const conv = buildConversion(query, env);
    const r = await client.uploadClickConversions([conv]);
    reportResult('upload', `order=${conv.orderId} gclid=${conv.gclid} ${conv.conversionValue} ${conv.currencyCode}`, r);
  ...
  if (query.order_reference && query.commissions != null) {
    ...
    reportResult('restate', `order=${query.order_reference} ${money(query.commissions)} RON`, r);
